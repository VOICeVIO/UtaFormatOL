    ...
    public class Data
    {
        ...
        public bool Import(List<string> filenames)
        {
            ...
            switch (format)
            {
                ...
                case UtaFormat.Vsq3:
                case UtaFormat.Vsq4:
                    return ImportVsqx(File.OpenRead(Files[0]), Path.GetFileName(Files[0]));
                case UtaFormat.Ust:
                    return ImportUst(Files.Select(File.OpenRead).Cast<Stream>().ToList(),
                        Files.Select(Path.GetFileName).ToList());
                case UtaFormat.Ccs:
                    return ImportCcs(File.OpenRead(Files[0]), Path.GetFileName(Files[0]));
            }
            return false;
        }

        public bool ImportVsqx(Stream stream, string fileName)
        {
            XDocument vsq = XDocument.Load(stream);
            bool isVsq3 = false;
            var nameSpace = (vsq.FirstNode as XElement)?.GetDefaultNamespace();
            if (nameSpace == null)
            {
                isVsq3 = vsq.ToString().Contains(Vsq3NameSpace);
            }
            else
            {
                isVsq3 = nameSpace.ToString().Contains(Vsq3NameSpace);
            }

            return isVsq3 ? ImportVsq3(vsq, fileName) : ImportVsq4(vsq, fileName);
        }

        public bool ImportVsq3(XDocument vsq, string fileName)
        {
            //Set up tracklist
        ...
        public bool ImportVsq4(XDocument vsq, string fileName)
        {
        ...
        public bool ImportUst(IList<Stream> streams, IList<string> fileNames)
        {
            int trackcount = 0;
            TrackList = new List<Track>();
            for (var i = 0; i < fileNames.Count; i++)
            {
                ...
                var ustValid = false;
                var ustReader = new StreamReader(streams[i], Encoding.GetEncoding("Shift-JIS"));
                int noteNum = 0;
                int time = 0;
                PreMeasure = 1;
                //建立音轨列表
                ...
                //建立音符列表，读取音符信息
                for (string readBuf = "Starting";
                    readBuf != "[#TRACKEND]" && readBuf != null;
                    readBuf = ustReader.ReadLine())
                {
                    if (trackcount == 0)
                    {
                    ...
                    if (readBuf.Contains("[#0000]"))
                    {
                        ...
                        for (readBuf = ustReader.ReadLine();
                            readBuf != "[#TRACKEND]" && readBuf != null;
                            readBuf = ustReader.ReadLine())
                        {
                            if (readBuf.Contains("[#"))
                            ...
                            {
                                if (readBuf.Substring(6, readBuf.Length - 6) != "R" &&
                                    readBuf.Substring(6, readBuf.Length - 6) != "r" && newNote.GetNoteLength() != 0)
                                {
                                    noteIsValid = true;
                                    ustValid = true;
                            ...
                            if (trackcount == 0)
                            {
                                if (readBuf.Contains("Tempo="))
                                {
                                    ...
                                    newTempo.BpmTimes100 =
                                        (int) (100 * double.Parse(readBuf.Substring(6, readBuf.Length - 6)));
                                    TempoList.Add(newTempo);
                                }
                            }
        ...
        public bool ImportCcs(Stream stream, string fileName)
        {
            XDocument ccs = XDocument.Load(stream);
        ...
        public Dictionary<string, string> ExportUst(out string omitList)
        {
            omitList = "";
            ...
            for (int tracknum = 0; tracknum < TrackList.Count; tracknum++)
            {
                ...
                contents.Add(TrackList[tracknum].TrackName, ustcontents);
